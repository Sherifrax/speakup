...
export const speakupEntrySearch = apiService.injectEndpoints({
  endpoints: (builder) => ({
    ...
    >({
      query: ({ search, pagination }) => {
        ...
        return {
          url: baseModule.speakup + speakupUrls.Search,
          method: "POST",
          params,
          body,
        };
...
export const { useSearchSpeakupMutation } = speakupEntrySearch;
